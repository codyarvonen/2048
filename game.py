...
class Game:
    ...
    def __init__(
        ...
        if initial_board is not None:
            assert initial_board.shape == (
                board_size,
                board_size,
            ), "Initial board size must have size (board_size, board_size)"

        if iterative_mode:
            assert (
                command_list is None
            ), "In iterative mode, no command list can be specified"
        ...
        if not visualize and not iterative_mode:
            ...
            ), "A list of commands must be provided for headless mode"

        if save_game:
            assert (
                seed is not None
            ), "Must specify a random seed to save the game history"
